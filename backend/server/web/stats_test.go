...
func (m *MockStatsDB) GetMostUsedDomains(
...
func (m *MockStatsDB) GetUsedDomainsByTimeAggregation(
...
func (m *MockStatsDB) GetMostUsedDomainsByTimeAggregation(
...
func (m *MockStatsDB) GetServerUsageByTimeRange(
	ctx context.Context, from, to time.Time, categories []string,
...
func TestHTTP_getServerUsageByTimeRangeDashboard_InvalidJSON(t *testing.T) {
	...
	req := httptest.NewRequest("POST", "/api/v1/dashboard/server-usage-by-time-range", bytes.NewBuffer(invalidJSON))
	req.Header.Set("Content-Type", "application/json")
